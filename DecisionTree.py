...
def main():
	...
	hyper_params.append(('entropy', 10, 0.0001))
	# hyper_params.append(('gini', 10, 0.0001))
...
def createModel(hyper_params, train_dataset, train_labels, test_dataset, test_labels):
	criterion = hyper_params[0]
	max_depth = hyper_params[1]
	min_impurity_decrease = hyper_params[2]
	
	print "Starting to create Decision Tree with %s ..." % (str(hyper_params)) 
	classifier = DecisionTreeClassifier(criterion=criterion, max_depth=max_depth, min_impurity_decrease=min_impurity_decrease)
